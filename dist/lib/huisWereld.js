...
// naar te kijken.
export function dagtijdVan(nu = new Date()) {
    const u = nu.getHours();
    if (u >= 5 && u < 9)
        return "ochtend";
    if (u >= 9 && u < 17)
...
const bloem = (x, y, k) => `<circle class="hw-bl hw-bl${k}" cx="${x}" cy="${y}" r="2.8"/><path class="hw-steel" d="M${x} ${y + 3}v9"/>`;
export function buitenSvg() {
    // ── laag 1: ver weg ───────────────────────────────────────────────
    const ver = `<g class="lg lg-1">` +
    ...
    let planken = "";
    for (let x = 244; x <= 300; x += 8)
        planken += `M${x} 308V410`;
    const huis = `<g class="lg lg-2">` +
    ...
    const stenen = [
        ...
        .join("");
    const heg = (x, b) => `<g class="hw-heg"><rect class="hw-heg-vlak" x="${x}" y="422" width="${b}" height="28" rx="12"/>` +
        Array.from({ length: Math.floor(b / 16) }, (_, i) => `<circle class="hw-heg-vlak" cx="${x + 8 + i * 16}" cy="424" r="8"/>`).join("") +
        `</g>`;
    const boom = (x, y, s, vertraging) => `<g transform="translate(${x} ${y}) scale(${s})"><g class="hw-boom" style="animation-delay:${vertraging}s">` +
        ...
        `</g></g>`;
    const lantaarn = (x) => `<path class="hw-lijn" d="M${x} 452V418"/><rect class="hw-lamp-kap" x="${x - 5}" y="410" width="10" height="9" rx="2"/>` +
        `<circle class="hw-lamp-gloed hw-lamp-gloed--groot" cx="${x}" cy="414" r="20"/><circle class="hw-lamp" cx="${x}" cy="414" r="3"/>`;
    const bloemen = [24, 40, 56, 72].map((x, i) => bloem(x, 470 - (i % 2) * 4, (i % 4) + 1)).join("") +
        [268, 284, 300, 316, 332].map((x, i) => bloem(x, 468 - (i % 2) * 4, ((i + 1) % 4) + 1)).join("");
    const tuin = `<g class="lg lg-3">` +
    ...
    const vuurvliegjes = `<g class="hw-vuurvliegjes hw-vuurvliegjes--vrij"><circle cx="150" cy="396" r="1.8"/><circle cx="266" cy="410" r="1.8"/>` +
        `<circle cx="320" cy="430" r="1.8"/><circle cx="40" cy="450" r="1.8"/><circle cx="110" cy="470" r="1.8"/></g>`;
    return (`<svg class="huis-buiten-svg" viewBox="0 0 400 560" preserveAspectRatio="xMidYMax meet" role="group" aria-label="Je huis van buiten">` +
        ver +
        huis +
        tuin +
        vuurvliegjes +
