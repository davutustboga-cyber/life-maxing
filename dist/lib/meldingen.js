...
const VAPID_PUBLIC_KEY = "BDQn6Yks6hiA1WbR5x0yp2J8MCEFTal_tan49mzzT775AsO2hyFzuwyAlXCmtFRJCUBLp3G8tjk5mTkh1QuTfmQ";
const ABONNEER_URL = "https://ksatpgclqfdyvinslrjk.supabase.co/functions/v1/abonneren";
export function meldingenOndersteund() {
    return "serviceWorker" in navigator && "PushManager" in window && "Notification" in window;
}
export function meldingenGeblokkeerd() {
    return meldingenOndersteund() && Notification.permission === "denied";
}
function urlBase64NaarUint8Array(base64Url) {
...
function naarServerFormaat(tijden) {
    const resultaat = {};
    if (tijden.ochtend)
        resultaat.ochtend = tijden.ochtend;
    if (tijden.middag)
        resultaat.middag = tijden.middag;
    if (tijden.avond)
        resultaat.avond = tijden.avond;
    return resultaat;
}
...
export async function zetMeldingenAan(tijden) {
    if (!meldingenOndersteund())
        return { ok: false, reden: "niet_ondersteund" };
    try {
        ...
        if (!subscription) {
            subscription = await registratie.pushManager.subscribe({
                ...
                applicationServerKey: urlBase64NaarUint8Array(VAPID_PUBLIC_KEY),
            });
        }
