...
export function objectenAan(data) {
    ...
    const aantal = {};
    const geteld = new Set();
    for (const ster of data.sterren ?? []) {
        const k = kamerVanSter(ster, data);
        const activiteit = ster.bewegingId ?? (ster.momentId ? data.momenten.find((m) => m.id === ster.momentId)?.gekozenDeur : undefined) ?? ster.id;
        const sleutel = `${k}|${activiteit}|${ster.datum}`;
        if (geteld.has(sleutel))
            continue;
        geteld.add(sleutel);
