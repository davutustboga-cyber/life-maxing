...
export function suggestiesVoorNu(data, nu = new Date()) {
    ...
    const beweging = (id, waarom) => bewegingSuggestie(id, waarom, gedaan);
    const vast = []; // wekelijks en vaste rituelen
    let pool = [];
    switch (dagdeel) {
        case "vroege_ochtend":
        case "ochtend": {
            ...
            pool = [
                beweging("ochtendlicht-zien", chips.has("slecht_geslapen")
                ...
                beweging("vijf-minuten-naar-buiten", chips.has("niet_buiten_geweest") || chips.has("niet_bewogen")
