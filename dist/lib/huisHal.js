...
export function deurMidden(kamer) {
    const d = HAL_DEUREN.find((k) => k.kamer === kamer);
    return d ? { x: d.x + d.b / 2, y: d.y + d.h / 2 } : { x: 50, y: 50 };
}
export function halSvg() {
    let posten = "";
    for (let x = -590; x < 950; x += 22)
        posten += `M${x} 198v16`;
    let treden = "";
    for (let i = 1; i < 9; i++)
        treden += `M0 ${442 - i * 24}L${(i * 46) / 9 + 2} ${442 - i * 24}`;
    return (`<svg class="hal-svg" viewBox="0 0 360 500" preserveAspectRatio="none" aria-hidden="true">` +
        ...
        // muren en galerij
        `<rect class="hl-wand" x="-600" y="-600" width="1560" height="1100"/>` +
        `<rect class="hl-lambrisering" x="-600" y="150" width="1560" height="64"/><rect class="hl-lambrisering" x="-600" y="376" width="1560" height="66"/>` +
        `<rect class="hl-slab" x="-600" y="214" width="1560" height="12"/>` +
        `<path class="hl-rail" d="M-600 198H960${posten}"/>` +
        // licht van boven
        ...
        // vloer, kleed
        `<rect class="hl-vloer" x="-600" y="442" width="1560" height="700"/><path class="hl-plint" d="M-600 442H960"/>` +
