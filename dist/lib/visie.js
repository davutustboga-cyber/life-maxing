...
export function visieCheckInsVoor(data) {
...
export function visieBeschikbaarAlsIntro(data) {
    return !(data.instellingen.visieIntroAangeboden ?? false);
}
export function registreerVisieIntroAangeboden(data) {
    data.instellingen.visieIntroAangeboden = true;
}
export function schrijfVisie(periode, wieIkBen, watIkHeb, waarIkSta, bestaand, nu = new Date()) {
    return {
        periode,
...
export function visieDelen(visie, islamAan) {
    return VISIE_DELEN.filter((d) => !d.islamitisch || islamAan)
        .map((def) => ({ def, tekst: (visie[def.veld] ?? "").trim() }))
        .filter((d) => d.tekst.length > 0);
}
export function visieTeksten(visie) {
    const uit = {};
    for (const d of VISIE_DELEN)
        uit[d.veld] = (visie?.[d.veld] ?? "").trim();
    return uit;
}
/** Schrijft de vijfjaarsvisie (of past hem aan). Geen geschiedenis (Wet 4). */
export function schrijfVisieDelen(teksten, bestaand, nu = new Date()) {
    const nieuw = {
    ...
    for (const d of VISIE_DELEN)
        nieuw[d.veld] = (teksten[d.veld] ?? "").trim();
    return nieuw;
}
/**
 ...
 */
export function visieFragment(visie, nu = new Date(), islamAan = true) {
    const delen = visieDelen(visie, islamAan).map((d) => ({ label: d.def.stam, tekst: d.tekst }));
    if (delen.length === 0)
        return null;
    const dagVanJaar = Math.floor((Date.UTC(nu.getFullYear(), nu.getMonth(), nu.getDate()) - Date.UTC(nu.getFullYear(), 0, 0)) / 86400000);
...
export function visieFragmentZichtbaar(data, dagdeel) {
    if (!data.visie)
        return false;
    const checkIns = visieCheckInsVoor(data);
    if (dagdeel === "ochtend" || dagdeel === "vroege_ochtend")
        return checkIns.ochtend;
    if (dagdeel === "middag")
        return checkIns.middag;
    return false;
}
// ── De herschrijfhulp ────────────────────────────────────────────────────
...
export function toekomstSignaal(tekst) {
