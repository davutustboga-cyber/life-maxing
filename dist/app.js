    ...
    render([
        el("div", { class: "scherm" }, [
            ...
            beweging.medischeGrens
                ? el("div", { class: "herkomst" }, beweging.medischeGrens.map((regel) => el("p", { class: "herkomst-regel" }, [regel])))
                : null,
...
function toonS19KleinsteStap() {
    render([
        el("div", { class: "scherm" }, [
            ...
            el("div", { class: "brieven-lijst" }, bewegingen.map((b) => el("button", { class: "brief-regel", onclick: () => toonS19Beweging(b.id) }, [b.titel]))),
...
function toonS23AvondSluiten() {
    ...
    const visieRegels = data.visie
        ...
        : [];
    const visieBlok = data.visie && visieCheckInsVoor(data).avond && visieRegels.length > 0
        ? el("div", { class: "herkomst" }, visieRegels.map((r) => el("p", { class: "herkomst-regel" }, [r])))
        : null;
    function vulChips() {
        chipsGrid.replaceChildren(...teksten.avondSluiten.chips.map((c) => el("button", {
            ...
            onclick: () => {
                if (gekozenChips.has(c.id))
                    gekozenChips.delete(c.id);
                else
                    gekozenChips.add(c.id);
                vulChips();
            },
        }, [c.label])));
    }
    ...
    function klaar() {
        ...
        data.dagsluitingen.push({
            ...
            positie,
            chips: [...gekozenChips],
            dankbaarheid: dankVeld.value.trim() || null,
            zin: zinVeld.value.trim() || null,
            voorMorgen: voorMorgenVeld.value.trim() || null,
        });
        void bewaren();
...
function navBalk(actief) {
    const item = (tab, label, actie) => el("button", { class: `nav-item${actief === tab ? " actief" : ""}`, onclick: actie }, [label]);
...
function voerSuggestieUit(s) {
    switch (s.soort) {
        ...
        case "beweging":
            if (s.id)
                toonOefening(s.id, { opKlaar: () => toonKlaar(), opTerug: () => toonThuis() });
            return;
        case "dhikr":
            if (s.id)
