 ...
 */
function opMaat(canvas: HTMLCanvasElement, resize: () => void): () => void {
...
export interface HemelOpties {
  ...
  rustig?: boolean;
  /**
   * v27 — de ster die bij het openen kort oplicht: die van vandaag, of (na het
   * afronden van een handeling, zie toonS7) de ster die zojuist ontstond.
   * Uit bij rustige beelden.
   */
  oplichtSterId?: string | null;
  /** Duur van het oplichten in ms (standaard 1200). */
  oplichtMs?: number;
  /** Wachttijd in ms voor het oplichten begint (standaard 250). */
  oplichtVertraging?: number;
}
 ...
 * daarna nooit meer.
 */
interface AchtergrondSter {
  x: number;
  y: number;
  r: number;
  basisAlpha: number;
  twinkel: number;
  /** Bijna alle sterren zijn wit; een klein deel krijgt een vleugje van de
   * merktinten, voor dezelfde reden als een echte lange-belichtingsfoto van
   * de nachthemel er nooit uniform wit uitziet. */
  kleur: string;
}

const STERKLEUREN: [aandeel: number, rgb: string][] = [
  [0.82, "237, 233, 222"], // wit — de meerderheid
  [0.1, "212, 161, 58"], // flauw messing
  [0.08, "139, 155, 192"], // flauw dusk-blauw
];

function kiesSterkleur(random: () => number): string {
  const r = random();
  let cumulatief = 0;
  for (const [aandeel, rgb] of STERKLEUREN) {
    cumulatief += aandeel;
    if (r <= cumulatief) return rgb;
  }
 ...
 */
function genereerAchtergrondSterren(breedte: number, hoogte: number): AchtergrondSter[] {
...
function tekenAtmosfeer(
  ...
  const vlekken: { kleur: string; x0: number; y0: number; r: number; snelheid: number; fase0: number }[] = [
 ...
 */
function tekenMelkweg(ctx: CanvasRenderingContext2D, breedte: number, hoogte: number): void {
 ...
 */
function volgendeValMoment(nu: number): number {
  return nu + 16000 + Math.random() * 22000;
}

function spawnVallendeSter(breedte: number, hoogte: number, nu: number): VallendeSter {
...
function tekenVallendeSterren(ctx: CanvasRenderingContext2D, sterren: VallendeSter[], nu: number): void {
...
function leeftijdsRang(sterren: Ster[]): Map<string, number> {
 ...
 */
function maakAchtergrondLaag(
  breedte: number,
  hoogte: number,
  stilleSterren: AchtergrondSter[]
...
export function tekenHemel(
  ...
  const {
    sterrenbeelden = [],
    nieuwSterrenbeeldId = null,
    rustig = false,
    oplichtSterId = null,
    oplichtMs = 1200,
    oplichtVertraging = 250,
  } = opties;
  ...
  let laag = maakAchtergrondLaag(
    rect.width,
    rect.height,
    achtergrond.filter((_, i) => i % 7 !== 0)
  );
  let raf = 0;
  let fase = 0;
  ...
  let vallendeSterren: VallendeSter[] = [];
  ...
  let tikPuls: { x: number; y: number; begonnenOp: number } | null = null;
  ...
  function herteken() {
    ...
    for (const s of tekenVolgorde) {
      ...
      if (oplicht !== null) helderheid = Math.min(1, 0.25 + 0.9 * (1 - Math.pow(1 - oplicht, 2)));
      ...
      if (!rustig) {
        ...
        if (vlamFase > 0.965) helderheid += ((vlamFase - 0.965) / 0.035) * 0.45;
      }
      ...
      if (voltooiGepulst) {
        ...
        if (isVanNieuw) {
          ...
          if (tv >= 0 && tv <= 1) helderheid += (1 - tv) * 0.6;
        }
      }
      ...
      if (tikPuls) {
        ...
        if (dichtbij) {
          ...
          if (tt >= 0 && tt <= 1) helderheid += (1 - tt) * 0.5;
        }
      }
  ...
  function clickHandler(e: MouseEvent) {
    ...
    if (ster) {
      ...
      if (p) tikPuls = { x: p.x, y: p.y, begonnenOp: performance.now() };
    }
  }
