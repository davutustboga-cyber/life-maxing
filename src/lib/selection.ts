...
import type { Moment, Onderdrukking, Tijd, Zone, LifeMaxingData } from "./types.js";
...
export function bepaalZone(
  woordIds: string[],
  eigenPositie?: { energie: number; toon: number }
): Zone {
  const gekozenWoorden = woordIds.map(woordById).filter((w): w is NonNullable<typeof w> => !!w);
...
function isOnderdrukt(bewegingId: string, onderdrukkingen: Onderdrukking[]): boolean {
  return onderdrukkingen.some((o) => o.bewegingId === bewegingId && new Date() < new Date(o.totDatum));
}

function drieZwareDagenEnRichting(momenten: Moment[]): boolean {
  const laatste3 = momenten.slice(-3);
  if (laatste3.length < 3) return false;
 ...
 */
function netOverprikkeldOfUitgeput(momenten: Moment[]): boolean {
 ...
 */
function directNaIetsPijnlijks(momenten: Moment[]): boolean {
  const vorige = momenten[momenten.length - 1];
  if (!vorige) return false;
...
function lostSlotOp(
  ...
  const geldigeOpties = slot.filter((optie) => {
    if (optie.vereistIslamitischeLaag && !islamitischeLaag) return false;
    if (optie.vereistWoord && !optie.vereistWoord.some((w) => gekozenWoorden.has(w))) return false;
    ...
    if (optie.vereistDagvenster === "ochtend" && !ochtend) return false;
    return true;
  });
  if (geldigeOpties.length === 0) return slot[0].id; // kan niet gebeuren zolang elke rotatie een ongeclausuleerde optie heeft
  if (geldigeOpties.length === 1) return geldigeOpties[0].id;

  const idsInRotatie = new Set(slot.map((o) => o.id));
  const vorigeKeuzes = data.momenten
    .map((m) => m.gekozenDeur)
    .filter((id): id is string => !!id && idsInRotatie.has(id));
...
export function bepaalDeuren(
  ...
  if (zone === "C_ordenen" && config.vervangingen) {
    for (const vervanging of config.vervangingen) {
      if (vervanging.nooitBijTijd?.includes(tijd)) continue;
  ...
  const sessieDeuren = new Set(data.momenten.slice(-1).map((m) => m.gekozenDeur).filter(Boolean) as string[]);
  const gekozenWoorden = new Set(woordIds);

  const geldig = kandidaten.filter((id) => {
    if (nooit.has(id)) return false;
    if (isOnderdrukt(id, data.onderdrukkingen)) return false;
    if (sessieDeuren.has(id)) return false;
    const beweging = bewegingById(id);
    if (!beweging) return false;
    // bewegingen.yaml → past_niet_bij: dit is de regel die "tien minuten
    // wandelen" aanbood aan iemand die "uitgeput" had gekozen.
    if (beweging.pastNietBij.some((w) => gekozenWoorden.has(w))) return false;
    // duur mag nooit langer zijn dan gevraagd
    if (tijd === "2min" && beweging.kosten.tijdMinuten[0] > 5) return false;
    for (const regel of beweging.nooitAanbiedenAls) {
      if (regel === "drie_zware_dagen_en_richting" && drieZwareDagenEnRichting(data.momenten)) return false;
      if (regel === "net_overprikkeld_of_sociaal_uitgeput" && netOverprikkeldOfUitgeput(data.momenten)) return false;
      if (regel === "direct_na_iets_pijnlijks" && directNaIetsPijnlijks(data.momenten)) return false;
    }
    return true;
  });
  ...
  while (geldig.length < 2) {
    const extra = fallbacks.find((f) => {
      if (geldig.includes(f) || nooit.has(f)) return false;
  ...
  geldig.sort((a, b) => {
    const past = (id: string) =>
      bewegingById(id)!.pastBij.filter((w) => gekozenWoorden.has(w)).length;
...
export function registreerOnderdrukking(
  data: LifeMaxingData,
  bewegingId: string,
