...
import type { LifeMaxingData, Maandbrief, Moment } from "./types.js";
 ...
 */
function eigenZinnen(momenten: Moment[]): string[] {
...
export function schrijfBrief(maand: string, momenten: Moment[]): Maandbrief {
...
export function vulBrievenAan(data: LifeMaxingData, nu = new Date()): boolean {
  ...
  for (const m of data.momenten) {
    ...
    if (sleutel >= huidigeMaand) continue;
