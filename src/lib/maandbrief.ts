...
import type { LifeMaxingData, Maandbrief, Moment, Ster } from "./types.js";
...
function sterkteInWoorden(deel: number): string | null {
  if (deel >= 0.8) return "bijna altijd";
  if (deel >= 0.6) return "vaak";
  if (deel >= 0.35) return "soms";
  return null;
}
...
export function maandNaam(sleutel: string): string {
  ...
  if (!naam) return sleutel;
  return naam.charAt(0).toUpperCase() + naam.slice(1);
}

function toonVanMoment(moment: Moment): number | null {
  if (moment.eigenWoord) return moment.eigenWoord.toon;
...
function toonVanAfsluiting(moment: Moment): number | null {
 ...
 */
function verschuivingszin(momenten: Moment[]): string {
  const perBeweging = new Map<string, { totaal: number; lichter: number }>();

  for (const m of momenten) {
    const id = m.gekozenDeur;
    if (!id || id === "niets-doen") continue;
    const voor = toonVanMoment(m);
    const na = toonVanAfsluiting(m);
    if (voor === null || na === null) continue;
    const rij = perBeweging.get(id) ?? { totaal: 0, lichter: 0 };
    rij.totaal += 1;
    if (na > voor + 0.05) rij.lichter += 1;
    perBeweging.set(id, rij);
  }

  let beste: { id: string; totaal: number; lichter: number } | null = null;
  for (const [id, rij] of perBeweging) {
    if (rij.totaal < SPIEGEL_DREMPEL) continue;
    if (!beste || rij.totaal > beste.totaal) beste = { id, ...rij };
  }
  if (!beste) return VAST.verschuivingOnbekend;

  const sterkte = sterkteInWoorden(beste.lichter / beste.totaal);
  if (!sterkte) return VAST.verschuivingOnbekend;

  const titel = bewegingById(beste.id)?.titel;
  if (!titel) return VAST.verschuivingOnbekend;
 ...
 */
function eigenZinnen(sterren: Ster[]): string[] {
  ...
  if (zinnen.length <= 4) return zinnen;
...
export function schrijfBrief(maand: string, momenten: Moment[], sterren: Ster[] = []): Maandbrief {
  ...
  if (zinnen.length > 0) alineas.push(zinnen.join(" "));
 ...
 * bewaren.
 */
export function vulBrievenAan(data: LifeMaxingData, nu = new Date()): boolean {
  const huidigeMaand = `${nu.getFullYear()}-${String(nu.getMonth() + 1).padStart(2, "0")}`;
  const bestaand = new Set((data.brieven ?? []).map((b) => b.maand));

  const relevant = (sleutel: string) => sleutel < huidigeMaand && !bestaand.has(sleutel);

  const perMaand = new Map<string, Moment[]>();
  for (const m of data.momenten) {
    const sleutel = maandSleutel(m.tijdstip);
    if (!relevant(sleutel)) continue;
  ...
  const sterrenPerMaand = new Map<string, Ster[]>();
  ...
  if (perMaand.size === 0) return false;

  const nieuwe = [...perMaand.entries()]
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([maand, momenten]) => schrijfBrief(maand, momenten, sterrenPerMaand.get(maand) ?? []));

  data.brieven = [...(data.brieven ?? []), ...nieuwe].sort((a, b) =>
    a.maand.localeCompare(b.maand)
  );
  return true;
}

export function ongelezenBrief(data: LifeMaxingData): Maandbrief | null {
 ...
 */
export function briefOpschrift(brief: Maandbrief, alle: Maandbrief[]): string {
