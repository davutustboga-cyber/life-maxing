...
if ("serviceWorker" in navigator) {
  ...
  navigator.serviceWorker.addEventListener("controllerchange", () => {
    if (herladenAlBezig || !wasAlBeheerd) return;
    const typt = [...document.querySelectorAll<HTMLTextAreaElement | HTMLInputElement>("textarea, input[type=text]")].some((v) => v.value.trim());
    if (typt) return;
