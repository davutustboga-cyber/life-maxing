...
import type { Beweging, Ster, Tijd } from "./lib/types.js";
...
import { glyphs, type GlyphNaam } from "./lib/glyphs.js";
import { kamers, kamerById, kamerVanBeweging, type Kamer, type KamerId } from "./data/kamers.js";
...
import { profetenVerhalen, motivatiehoekVoorVandaag, type MotivatiehoekDag, type ProfetenVerhaal } from "./data/motivatiehoek.js";
import {
  begroeting,
  dagdeelVan,
  dagdeelGroep,
...
import {
  ...
  toonMelding,
  zichtbareBewegingen,
  startMomentUitCheckIn,
} from "./app.js";

// ── bouwstenen ─────────────────────────────────────────────────────────

type Kind = Parameters<typeof el>[2] extends (infer K)[] | undefined ? K : never;

function glyph(naam: GlyphNaam, klasse = ""): ReturnType<typeof el> {
...
function rij(titel: string, onder: string | null, actie: () => void): ReturnType<typeof el> {
...
function kamerBalk(opTerug: () => void, rechts?: Kind): ReturnType<typeof el> {
  return el("div", { class: "kamer-balk" }, [terugKnop(opTerug), rechts ?? null]);
}

function kamerKop(kamer: Kamer, titel = kamer.naam, regel: string | null = kamer.doel): ReturnType<typeof el> {
 ...
 */
let kamerHerkomst: "thuis" | "kamer" = "thuis";
let laatsteKamer: KamerId | null = null;
function terugNaarHerkomst(): void {
  if (kamerHerkomst === "kamer" && laatsteKamer) toonKamerBinnen(laatsteKamer);
  else toonThuis();
}

/** Alle deuren blijven zichtbaar; Geloof met de laag uit is gedimd en opent een uitleg met een aan-knop. */
function zichtbareKamers(): Kamer[] {
  return kamers;
}

/** Naar de hal, of via "terug" naar een kamer: kort vloeiend, zonder animatie te forceren. */
export function toonKamer(id: KamerId | string): void {
  if (kamerById(id)) laatsteKamer = id as KamerId;
  switch (id) {
    case "visie":
...
function kamerVanSuggestie(s: Suggestie): Kamer | null {
  switch (s.soort) {
    case "beweging": {
 ...
 */
function visieHero(): ReturnType<typeof el> {
...
export function toonThuis(): void {
  kamerHerkomst = "thuis";
  ruimSessieOp();
  const nu = new Date();
  ...
  let stilleRegel: string | null = null;
  ...
  if (huisNieuw.length > 0) {
    ...
    if (k) stilleRegel = nieuwObjectZin(eerste.kamer, k.naam, eerste.nummer);
    ...
    const kerntaak =
      groep === "middag" || groep === "avond" ? ochtendMomentVandaag(data, nu)?.kerntaak?.trim() || null : null;
    if (voorVandaag) stilleRegel = `${teksten.nuRegels.voorMorgen}: ${voorVandaag}`;
    else if (kerntaak) stilleRegel = `${teksten.nuRegels.kerntaak}: ${kerntaak}`;
    else {
      const eigen = dagVanJaar(nu) % 3 === 0 ? eigenZinVanEerder(data, nu) : null;
      if (eigen) stilleRegel = eigen;
        ...
        if (f && hero && f.tekst !== hero.tekst) stilleRegel = `${f.label} ${f.tekst}`;
      }
    }
  }
  ...
  render([
    el("div", { class: "scherm scherm-app scherm-hal", "data-kamer": "vandaag" }, [
      ...
      el(
        ...
        zichtbareKamers().map((k, i) => {
          const deur = el("button", { class: `kamerdeur${k.islamitisch && !islamAan() ? " kamerdeur--uit" : ""}`, "data-kamer": k.id, "aria-haspopup": "dialog", onclick: (e: Event) => toonKamerBlad(k, e.currentTarget as HTMLElement) }, [
...
function vandaagPaneel(
  ...
  const dockKnop = (soort: string, naam: GlyphNaam, tekst: string, actie: () => void) =>
    el("button", { class: `dock-knop dock-knop--${soort}`, onclick: actie }, [
 ...
 */
function toonHetGaatEvenNiet(): void {
  render([
...
function toonVerrasMe(vast?: Suggestie, uitsluiten: string[] = []): void {
  const s = vast ?? verrasMe(data, zichtbareBewegingen(), uitsluiten);
  if (!s || !s.id) return toonThuis();
  const id = s.id;
    ...
    el("div", { class: "scherm scherm-app scherm-kamer", "data-kamer": kamer?.id ?? "vandaag" }, [
      ...
      el(
        "button",
        {
          class: "nu-paneel",
          onclick: () =>
            toonBeweging(id, { opKlaar: () => toonVrijeAfronding(id), opTerug: () => toonVerrasMe(s, uitsluiten) }),
        },
        [
...
function bewegingRij(b: Beweging, opTerug: () => void): ReturnType<typeof el> {
  return rij(b.titel, `${duurTekst(b.id)} · ${b.minimumversie}`, () =>
    toonBeweging(b.id, { opKlaar: () => toonVrijeAfronding(b.id), opTerug })
  );
}

/** Alle bewegingen van één thema als rustige lijst, in de sfeer van de kamer. */
function toonThemaLijst(themaId: string, kamerId: KamerId): void {
  const t = themaById(themaId);
  if (!t) return toonKamer(kamerId);
  ...
  for (const bid of t.bewegingIds) {
    ...
    if (b) items.push(bewegingRij(b, opTerug));
  }
  if (islamAan() && t.dhikrIds) {
    for (const did of t.dhikrIds) {
      const d = dhikrById(did);
      if (d) items.push(rij(d.titel, d.wanneer, () => toonDhikr(did, opTerug)));
    }
  }
  render([
    kamerScherm(kamerId, [
...
export function toonVisieIntro(): void {
  render([
 ...
 */
export function toonVisieSchrijven(focus?: VisieDeelVeld): void {
  const delen = VISIE_DELEN.filter((d) => !d.islamitisch || islamAan());
  const huidig = visieTeksten(data.visie);
  const velden = new Map<VisieDeelVeld, HTMLTextAreaElement>();
  const hint = el("p", { class: "visie-hint" }, [""]);
  const bewaarKnop = el("button", { class: "knop" }, ["Bewaren"]) as HTMLButtonElement;

  const groei = (ta: HTMLTextAreaElement) => {
  ...
  const blokken = delen.map((d) => {
    const ta = el("textarea", { placeholder: d.placeholder, rows: "2", "aria-label": d.stam }) as HTMLTextAreaElement;
  ...
  bewaarKnop.addEventListener("click", () => {
    ...
    for (const [veld, ta] of velden) waarden[veld] = ta.value;
    // Delen die niet getoond werden (geloof met de laag uit) blijven zoals ze waren.
    for (const d of VISIE_DELEN) if (!velden.has(d.veld)) waarden[d.veld] = huidig[d.veld];
  ...
  for (const ta of velden.values()) groei(ta);
  verversKnop();
...
export function toonKamerVisie(): void {
...
const ADEM_ORBS: { id: string; naam: string; ritme: string }[] = [
...
function toonKamerAdem(): void {
  ...
  const b = (id: string, titel: string, onder: string | null) =>
    rij(titel, onder, () => toonBeweging(id, { opKlaar: () => toonVrijeAfronding(id), opTerug: terug }));
  render([
    kamerScherm("adem", [
      kamerBalk(terugNaarHerkomst),
...
const MENSEN_ZINNEN: { id: string; zin: string }[] = [
  ...
  render([
    kamerScherm("mensen", [
      ...
      el(
        "div",
        { class: "zinnen" },
        MENSEN_ZINNEN.map((z, i) => {
          const b = bewegingById(z.id);
          const knop = el(
            "button",
            { class: "zin", onclick: () => toonBeweging(z.id, { opKlaar: () => toonVrijeAfronding(z.id), opTerug: terug }) },
            [el("span", { class: "zin-tekst" }, [z.zin]), el("span", { class: "zin-onder" }, [b ? duurTekst(z.id) : ""])]
          );
...
const GELOOF_VOOR_NU: Record<string, string> = {
...
export function toonKamerGeloof(): void {
  if (!islamAan()) return toonGeloofUit();
  ...
  render([
    kamerScherm("geloof", [
      ...
      el(
        "div",
        { class: "rij-lijst" },
        adhkar.map((d) => rij(d.titel, d.wanneer, () => toonDhikr(d.id, () => toonDhikrLijst(), () => toonDhikrLijst())))
      ),
    ]),
  ]);
}
...
function toonProfetenLijst(): void {
  const groepen = new Map<string, ProfetenVerhaal[]>();
  for (const v of profetenVerhalen) {
    const lijst = groepen.get(v.profeet) ?? [];
    lijst.push(v);
    groepen.set(v.profeet, lijst);
  }
  const rijen = [...groepen.entries()].map(([profeet, lijst]) =>
    rij(profeet, lijst.length === 1 ? "1 verhaal" : `${lijst.length} verhalen`, () => toonProfeetVerhalen(profeet, () => toonProfetenLijst()))
  );
  render([
    kamerScherm("geloof", [
      kamerBalk(() => toonKamerGeloof()),
...
function toonProfeetVerhalen(profeet: string, opTerug: () => void): void {
  ...
  render([
    kamerScherm("geloof", [
      ...
      el(
        ...
        lijst.map((v, i) =>
          rij(v.titel, v.thema, () =>
            toonVerhaalLezer(v, hier, "geloof", i < lijst.length - 1 ? () => toonVerhaalLezer(lijst[i + 1], hier, "geloof") : undefined)
          )
        )
      ),
    ]),
...
function toonVerhaalLezer(v: ProfetenVerhaal, opTerug: () => void, kamer: KamerId, volgende?: () => void, volgendeTekst?: string): void {
  render([
...
function toonKamerMotivatie(): void {
...
function visieRegelVoorMotivatie(): string | null {
          ...
          "button",
          {
            ...
            onclick: (e: Event) => {
              ...
              if (i.soort === "enkel") {
                ...
                if (ouder) Array.from(ouder.children).forEach((c) => c.setAttribute("aria-pressed", "false"));
              }
...
function toonMotivatieSlot(): void {
  const zichtbaar = zichtbareBewegingen();
  const deuren = MOTIVATIE_DEUREN.map((id) => zichtbaar.find((b) => b.id === id)).filter((b): b is Beweging => Boolean(b));
      ...
      deur
        ? el(
            "button",
            {
              class: "nu-paneel",
              onclick: () =>
                toonBeweging(deur.id, { opKlaar: () => toonVrijeAfronding(deur.id), opTerug: () => toonMotivatieSlot() }),
            },
            [
...
function beginHier(stap: Suggestie, kamerId: KamerId): ReturnType<typeof el> {
  const terug = () => toonKamer(kamerId);
  const start = () => {
    if (!stap.id) return;
    if (stap.soort === "dhikr") toonDhikr(stap.id, terug, terug);
    else toonBeweging(stap.id, { opKlaar: () => toonVrijeAfronding(stap.id!), opTerug: terug });
  };
...
function kamerStart(id: KamerId, metStap = true): Kind[] {
...
function kamerVoet(): ReturnType<typeof el> {
...
// ── Gevoelscheck-in: gevoel → voorstel → kamer ─────────────────────────
// Vrijwillig en zonder uitleg: één keuze uit negen. Een gevoel is een
// aanwijzing voor een mogelijke behoefte, geen diagnose; er wordt niets
// bijgehouden (het moment dat je begint is hetzelfde soort moment als in de
// kompaslus, zonder trend of teller).

export type CheckInContext = "ochtend" | "middag" | "avond" | "vrij";

const CHECKIN_KOP: Record<CheckInContext, { titel: string; regel: string }> = {
...
export function toonCheckIn(context: CheckInContext = "vrij"): void {
...
function toonCheckInAdvies(gevoelId: string, tijd: Tijd, context: CheckInContext): void {
  const advies = adviesVoor(gevoelId, tijd, data);
  if (!advies) return toonCheckIn(context);
  ...
  const tijdKeuze = metWoorden
    ? el("div", { class: "tijd-keuze", role: "group", "aria-label": "Hoeveel tijd heb je?" }, [
        (["2min", "10min_of_meer"] as Tijd[]).map((t) =>
          el(
            "button",
            {
              class: "tijd-pil",
  ...
  render([
    el("div", { class: "scherm scherm-app scherm-kamer", "data-kamer": kamer?.id ?? "vandaag" }, [
      ...
      el("div", { class: "rij-lijst" }, [
        ...
        metWoorden && zone
          ? rij("Andere opties bekijken", "De drie deuren, met “niets doen” erbij.", () =>
              startMomentUitCheckIn(gevoel.woorden, zone, tijd, "andere", terug)
            )
          : null,
      ...
      metWoorden && zone
        ? el(
            "button",
            { class: "knop-klein", onclick: () => startMomentUitCheckIn(gevoel.woorden, zone, tijd, "niets-doen", terug) },
            ["liever niets doen"]
          )
        : null,
    ]),
...
export function toonAvondRoutine(): void {
  ...
  if (islam) {
    ...
    for (const id of ["ayat-al-kursi", "sayyid-al-istighfar-tekst", "subhan-allahi-wa-bihamdihi"]) {
      ...
      if (d) items.push(rij(d.titel, d.wanneer, () => toonDhikr(id, terug, terug)));
    }
  }
  items.push(
    rij("Rustig ademen", "Vier tellen in, zes uit.", () =>
      toonBeweging("adem-lange-uitademing", { opKlaar: () => toonVrijeAfronding("adem-lange-uitademing"), opTerug: terug })
    )
  );
  render([
...
function toonKamerBlad(k: Kamer, opener: HTMLElement): void {
 ...
 */
function koppelKamerpassing(scene: HTMLElement, voet: HTMLElement): void {
  ...
  const ro = new ResizeObserver(() => {
    if (!scene.isConnected) return ro.disconnect();
    pas();
  });
  ro.observe(scene);
  ro.observe(voet);
...
export function toonKamerBinnen(id: KamerId): void {
  const kamer = kamerById(id);
  if (!kamer) return toonThuis();
  laatsteKamer = id;
  const aan = objectenAan(data)[id] ?? [];
...
// ── Het huis ────────────────────────────────────────────────────────────
// Je ziet je huis van buiten: voor iedereen hetzelfde, altijd mooi, en het
// verandert niet door wat je doet. Tik op de voordeur (of "Naar binnen") en de
// camera zoomt in op de deur; binnen sta je in een hal met zes deuren. Tik op
// een deur en de camera zoomt op die deur in, waarna de kamer opent. Wat je
// doet groeit binnen: zie lib/huis.ts en lib/kamerScene.ts. Er staat nergens
// een getal.
//
// Camera- en overgangsstaat, op één plek:
//   ruimte  "buiten" | "hal"      waar je bent
//   zoom    aan | uit             is de camera op een deur ingezoomd?
// Het scherm is een pure functie van die twee attributen op .wereld (zie
// style.css: [data-ruimte], [data-zoom]). JS zet nooit een transform zelf; het
// zet alleen die attributen en, voor de zoom, waar de deur zit (--cx, --cy).
// Daardoor kan er niets "blijven hangen": elke keer dat het huis opent wordt
// alles opnieuw opgebouwd met ruimte = buiten/hal en zoom = uit; een
// onderbroken overgang keert vanzelf om (CSS-transities), de terugknop
// annuleert een zoom, en bij `pageshow` (terug uit de cache) wordt de zoom
// gereset. De zoom is een vaste schaal (nooit gebruikersgestuurd), dus de
// camera kan niet buiten de tekening komen.

export type HuisRuimte = "buiten" | "hal";

function kamerStatusRegel(id: KamerId): string {
  const k = kamerById(id);
  if (!k) return "";
  if (id === "motivatie") return k.doel;
  const zin = laatsteZinInKamer(data, id);
  return zin ? `“${zin.zin}”` : trapZin(trappen(data)[id] ?? 0);
}

function rustigeBeelden(): boolean {
  return (
...
function koppelParallax(wereld: HTMLElement): void {
  if (rustigeBeelden()) return;
  let doelX = 0, doelY = 0, nuX = 0, nuY = 0, raf = 0;
  const loop = (): void => {
    if (!wereld.isConnected) { raf = 0; return; }
    nuX += (doelX - nuX) * 0.09;
    nuY += (doelY - nuY) * 0.09;
  ...
  const richt = (e: PointerEvent): void => {
...
function bouwHal(nieuw: Map<KamerId, number>, opKies: (id: KamerId, deur: HTMLElement) => void): HTMLElement {
  const trapNu = trappen(data);
...
export function toonHuis(start: HuisRuimte = "buiten"): void {
  // Wat er sinds de vorige keer is bijgekomen: één keer benoemd, dan gezien.
  const nieuw = new Map(nieuweObjecten(data).map((n) => [n.kamer, n.nummer] as [KamerId, number]));
  markeerHuisGezien(data);
  void bewaren();
  ...
  // ── camera ──
  const cam = { ruimte: start as HuisRuimte, zoom: false };
  let timer = 0;
  let geopend: HTMLElement | null = null;
  ...
  const pas = (): void => {
    ...
    if (cam.ruimte === "buiten") {
      sheet.replaceChildren(
        el("p", { class: "sheet-regel" }, ["Dit is jouw huis. Wat je doet, groeit binnen."]),
        el("button", { class: "knop", onclick: naarBinnen }, ["Naar binnen"])
      );
    } else {
      sheet.replaceChildren(
        el("p", { class: "sheet-regel" }, [k && eerste ? nieuwObjectZin(k.id, k.naam, eerste[1]) : "Kies een kamer."]),
        el("button", { class: "knop-klein", onclick: naarBuiten }, ["naar buiten"])
      );
    }
  ...
  function naarBinnen(): void {
    stabiel();
    cam.ruimte = "hal";
    pas();
  }
  function naarBuiten(): void {
    stabiel();
    cam.ruimte = "buiten";
    pas();
  }
  function naarKamer(id: KamerId, deur: HTMLElement): void {
    if (cam.zoom) return;
    ...
    timer = window.setTimeout(() => {
      if (wereld.isConnected) toonKamerBinnen(id);
    }, rustigeBeelden() ? 0 : 560);
  }
  // Na een sluimerstand of "terug" uit de cache: nooit ingezoomd blijven staan.
  const herstel = (): void => {
    if (!wereld.isConnected) return window.removeEventListener("pageshow", herstel);
    stabiel();
    pas();
  };
  ...
  buiten.addEventListener("click", (e) => {
    if ((e.target as Element | null)?.closest?.('[data-hv="binnen"]')) naarBinnen();
  });
  buiten.addEventListener("keydown", (e) => {
    if ((e.key === "Enter" || e.key === " ") && (e.target as Element | null)?.closest?.('[data-hv="binnen"]')) {
      e.preventDefault();
      naarBinnen();
    }
  });
  ...
  const terug = (): void => {
    if (cam.zoom) return stabiel();
    if (cam.ruimte === "hal") return naarBuiten();
    toonThuis();
  };
 ...
 */
export function toonHuisOplichten(kamerId: KamerId): void {
